...
const { copyHookScripts, verifyHookScriptCopies, removeHookScripts, removeMarkedSection, assertSafeConfigDir } = require('./hookAdapter');
...
function verify({ configRoot }) {
  ...
  if (pluginExists) {
    try {
      ...
      if (!stat.isFile()) {
        pluginLoadError = 'plugin path is not a regular file';
      } else {
        const installed = fs.readFileSync(pluginPath, 'utf8');
        const expected = buildPluginSource(hooksDir);
        pluginLoadable = installed === expected;
        if (!pluginLoadable) {
          pluginLoadError = 'plugin differs from the source generated by this Evolver version';
        }
      }
    } catch (err) {
  ...
  checks.push({
    ...
    detail: pluginLoadable
      ? 'plugin matches the generated Evolver source'
      : 'static verification failed: ' + (pluginLoadError || 'unknown'),
  });
